...
function inputBox(opts){return new Promise(res=>{_inpResolve=res;$('inpTitle').textContent=opts.title||'Input';const box=$('inpFields');box.innerHTML='';
 (opts.fields||[]).forEach(f=>{const w=document.createElement('div');w.style.margin='6px 0';if(f.group)w.dataset.group=f.group;if(f.type==='checkbox'){w.style.display='flex';w.style.alignItems='center';w.style.gap='8px';const cbx=document.createElement('input');cbx.id='inp_'+f.key;cbx.type='checkbox';cbx.checked=!!f.value;if(f.reveals){cbx.dataset.reveals=f.reveals;cbx.onchange=()=>box.querySelectorAll('[data-group="'+f.reveals+'"]').forEach(x=>{x.style.display=cbx.checked?'':'none';});}const clb=document.createElement('label');clb.textContent=f.label||f.key;clb.style.fontSize='13px';clb.htmlFor=cbx.id;clb.style.cursor='pointer';cbx.onkeydown=e=>{if(e.key==='Escape'){e.preventDefault();inpCancel();}};w.appendChild(cbx);w.appendChild(clb);box.appendChild(w);return;}const lb=document.createElement('label');lb.textContent=f.label||f.key;lb.style.display='block';lb.style.fontSize='12px';lb.style.marginBottom='2px';lb.style.color='var(--muted)';
  ...
  if(f.type==='password'){const wrap=document.createElement('div');wrap.style.position='relative';wrap.style.display='flex';const inp=document.createElement('input');inp.id='inp_'+f.key;inp.type='password';inp.style.width='100%';inp.style.paddingRight='28px';inp.style.boxSizing='border-box';
...
function openTab(title,sql,db,run,table,ddl){const id='t'+(++tabSeq);title=uniqueTabTitle(title||'Query');const tab={id,title,db:db||null,table:table||null,ddl:ddl||null,genSql:sql||'',sqlEdited:false,pk:null,cols:null,rows:null,limit:1000,offset:0,pending:null,filter:null,hiddenCols:new Set()};
 ...
 pane.innerHTML='<div class="edwrap" id="ew_'+id+'"><pre class="hl" id="hl_'+id+'"></pre><textarea class="editor" id="ed_'+id+'" spellcheck="false"></textarea></div>'+
  ...
  '<div class="toolbar"><span style="display:inline-grid"><button class="primary" id="runbtn_'+id+'" style="grid-area:1/1" title="Run the query (F5)" onclick="runTab(\''+id+'\')">Run Query</button><button class="warn" id="cancelbtn_'+id+'" style="grid-area:1/1;visibility:hidden" title="Cancel the running query" onclick="cancelQuery(\''+id+'\')">Cancel</button></span><button title="Run the selected text (Ctrl+Enter) - or, if nothing is selected, whichever statement the cursor is currently inside" onclick="runSel(\''+id+'\')" data-ic="runsel" data-fit="4">Run Selection</button><button title="Show how the server will run the current statement: the plan as a diagram, and the EXPLAIN table in the results" onclick="explainTab(\''+id+'\')" data-ic="explain" data-fit="4">Explain</button><button title="Reformat the query for readability (safe - only changes whitespace/line breaks, never the query itself)" onclick="formatTabSql(\''+id+'\')" data-ic="format" data-fit="3">Format</button>'+
  ...
  '<span class="tbsep"></span><span class="tbgroup runopts" style="display:inline-flex;align-items:center;gap:6px">'+
  '<button class="tog" id="coe_'+id+'" aria-pressed="false" onclick="togPress(this)" data-ic="skiperr" data-fit="1" title="Continue on error: if a statement fails, keep running the rest of the script instead of stopping at the first error - useful for bulk, mostly-independent statements like seed data or batch table creation. Every failure is reported, not just the first. Only applies to a script that does NOT end in a SELECT.">Continue on error</button>'+
