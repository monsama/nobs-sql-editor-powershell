...
$ast.FindAll({param($n) $n -is [System.Management.Automation.Language.FunctionDefinitionAst] -and
                        $n.Name -in @('Initialize-DumpDb','Test-SqlSafeToRerun','Test-SqlReadOnly','Test-SqlReadOnlyAs','Remove-SqlComments','Split-OffKeyword','Get-ExactTextMap','Strip-Parens')},$true) | ForEach-Object { Invoke-Expression $_.Extent.Text }
Initialize-DumpDb
